...
#
#    Nome completo: Paulo André Ribeiro Said
#    Matrícula: 202299417
#    Turma: CC3M
...
class Imagem:
    ...
    def get_pixel(self, x, y):
        ...
        A função calcula o índice correto do pixel com base nas coordenadas (x, y) e
        retorna o valor desse pixel na lista de pixels da imagem.
        """
        indice = (self.largura * y) + x
        return self.pixels[indice]

    def set_pixel(self, x, y, c):
        """
        Define o valor do pixel na posição (x, y) da imagem.

        Parâmetros:
            - x: a coordenada x do pixel.
            - y: a coordenada y do pixel.
            - c: o novo valor a ser atribuído ao pixel.

        A função calcula o índice correto do pixel com base nas coordenadas (x, y) e
        atribui o valor 'c' a esse pixel na lista de pixels da imagem.
        """
        indice = (self.largura * y) + x
    ...
    def aplicar_por_pixel(self, func):
        ...
        for x in range(self.largura):
            for y in range(self.altura):
                # Obtém a cor do pixel na posição (x, y) da imagem original
                cor = self.get_pixel(x, y)

                # Aplica a função lambda fornecida ao valor da cor
                nova_cor = func(cor)

                # Define a nova cor na posição (x, y) da imagem de resultado
                resultado.set_pixel(x, y, nova_cor)
        return resultado

    def invertida(self):
        return self.aplicar_por_pixel(lambda c: 255 - c) # O código aqui foi mudado: O valor máximo para um pixel em tons de cinza é 255, e não 256.
